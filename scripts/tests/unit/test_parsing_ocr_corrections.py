...
class TestOCRCorrections:
    ...
    def test_ocr_corrections_string_types(self):
        ...
        for error, correction in OCR_CORRECTIONS.items():
            ...
            assert isinstance(correction, str), f"Correction value should be string, got {type(correction)}"
    ...
    def test_ocr_corrections_no_empty_values(self):
        ...
        for error, correction in OCR_CORRECTIONS.items():
            assert len(correction) > 0, f"OCR correction value should not be empty for error: {error}"

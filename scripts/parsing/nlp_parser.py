...
def extract_healing_info(doc: spacy.tokens.Doc) -> dict:
    ...
    if heal_tokens:
        ...
        numbers = [token for token in doc if token.pos_ == "NUM" or (token.text.isdigit() and len(token.text) <= 2)]
...
def parse_power_levels_with_nlp(text: str) -> List[dict]:
    ...
    for i, line in enumerate(lines):
        ...
        if ("at the end" in line_lower or "atthe end" in line_lower) and "turn" in line_lower:
            power_start_idx = i
            break
        elif "heal" in line_lower and ("turn" in line_lower or "investigator" in line_lower or "space" in line_lower):
    ...
    for i, line in enumerate(power_lines):
        ...
        has_heal_and_turn = "heal" in line_lower and ("turn" in line_lower or "end" in line_lower or "investigator" in line_lower)
        ...
        if starts_with_instead and current_level_lines:
            ...
            if (
                any(keyword in line_lower for keyword in ["heal", "stress", "wound", "investigator", "space", "each", "and", "or"])

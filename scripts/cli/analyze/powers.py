...
class ElderSignConversion(BaseModel):
    ...
    successes_per_elder_sign: int = Field(
        default=1,
        ge=1,
        description="Number of successes each elder sign counts as (default: 1, can be 2)",
    )
    ...
    def from_description(cls, description: str) -> "ElderSignConversion":
        ...
        if match_any:
            ...
            if re.search(r"as\s+2\s+success", description, re.IGNORECASE):
                successes_per = 2
            return cls(
                elder_signs_as_successes=999, successes_per_elder_sign=successes_per
            )  # Special value for "any number"
...
class ActionAddition(BaseModel):
    ...
    actions_added: int = Field(default=0, ge=0, description="Number of free actions added per turn")
    action_type: ActionType = Field(default=ActionType.ATTACK, description="Type of action added")
...
class PowerLevelAnalysis(BaseModel):
    ...
    effect: str = Field(
        ...,
        description="What this power level does (e.g., 'Adds 1 green dice', 'Counts 1 elder sign as success')",
    )
...
def analyze_power_level(power_name: str, level: int, description: str) -> PowerLevelAnalysis:
    ...
    base_stats = calculator.calculate_combined_stats(BASE_BLACK_DICE_COUNT, BASE_GREEN_DICE_COUNT)
    ...
    if elder_sign_conversion.elder_signs_as_successes > 0:
        ...
        elder_bonus_successes = (
            elder_signs_converted * elder_sign_conversion.successes_per_elder_sign
        )
    ...
    if elder_sign_conversion.elder_signs_as_successes > 0:
        if elder_sign_conversion.converts_any_number:
            if elder_sign_conversion.successes_per_elder_sign > 1:
                effect_parts.append(
                    f"Counts any elder signs as {elder_sign_conversion.successes_per_elder_sign} successes each"
                )
            else:
            ...
            if elder_sign_conversion.successes_per_elder_sign > 1:
                effect_parts.append(
                    f"Counts {elder_sign_conversion.elder_signs_as_successes} elder sign(s) as {elder_sign_conversion.successes_per_elder_sign} successes each"
                )
            else:
                effect_parts.append(
                    f"Counts {elder_sign_conversion.elder_signs_as_successes} elder sign(s) as success(es)"
                )
...
def main(data_dir: Path, output_json: Optional[Path]):
    ...
    for analysis in all_analyses:
        ...
        if has_dice or has_elder:
            # Show effect summary
            effect_summary = (
                analysis.effect[:40] + "..." if len(analysis.effect) > 40 else analysis.effect
            )

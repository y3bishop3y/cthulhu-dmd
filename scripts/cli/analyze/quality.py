...
from scripts.core.parsing.text import OCR_CORRECTIONS
...
def check_statistics_consistency(level_data: CommonPowerLevelData) -> List[str]:
    ...
    has_green_dice_mention = any(
        phrase in description for phrase in dice_phrases if "green" in phrase.lower()
    )
    has_black_dice_mention = any(
        phrase in description for phrase in dice_phrases if "black" in phrase.lower()
    )
    ...
    if has_elder_sign_mention:
        if has_success_mention and "count" in description:
            ...
            if not has_elder_in_effect and not has_success_in_effect:
                issues.append(
                    "Description mentions counting elder signs as successes but effect doesn't reflect this"
                )

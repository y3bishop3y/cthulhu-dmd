...
try:
    ...
    print(
        ...
        "  1. uv run python scripts/parsing/parse_trait_character_assignments.py [options]\n"
        "  2. source .venv/bin/activate && python scripts/parsing/parse_trait_character_assignments.py [options]\n\n"
        "Recommended: uv run python scripts/parsing/parse_trait_character_assignments.py --help\n",
        file=sys.stderr,
    )
    sys.exit(1)
...
def main(data_dir: Path, output_json: Optional[Path]):
    ...
    if multi_trait_chars:
        ...
        for char, traits in sorted(
            multi_trait_chars.items(), key=lambda x: len(x[1]), reverse=True
        ):
    ...
    console.print(
        f"[cyan]Summary:[/cyan] {stats['total_traits']} traits, "
    ...
    if output_json:
        output_data = {
            "trait_characters": {
                trait_name: [
                    {"name": char.name, "number": char.number} for char in section.characters
                ]

...
def check_season_structure(season_dir: Path, season_name: str):
    ...
    if characters_subdir.exists():
        issues.append(f"Has nested 'characters/' subdirectory (should be flat)")
...
def main():
    ...
    print()
    print("=" * 70)
    print(f"Summary: {total_characters} total characters across {len([r for r in results if r['exists']])} seasons/boxes")

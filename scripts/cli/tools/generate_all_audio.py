...
def generate_audio_for_character(
    ...
    try:
        ...
        # Generate filename
        file_character_name = (
            character_name.lower().replace(" ", "_") if character_name else character
        )
...
def main(
    ...
    if speaker:
        try:
            ...
            console.print("[yellow]Warning: Could not load speaker metadata[/yellow]")
    ...
    with Progress(
        ...
        for character in characters:
            ...
            if skip_existing and not force:
                ...
                speaker_suffix = f"_{speaker}" if speaker else ""
                output_file = (
                    output_dir if output_dir else char_dir
                ) / f"{file_character_name}_audio{speaker_suffix}{OUTPUT_AUDIO_EXT}"
                if output_file.exists():

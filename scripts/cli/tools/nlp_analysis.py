...
def main(character: str, season: str, strategy: str):
    """Analyze OCR output with NLP to extract semantic meaning."""
    console.print("[bold cyan]NLP Analysis of OCR Output[/bold cyan]\n")
    ...
    if not back_path:
        console.print("[red]Error: Back card image not found[/red]")
        sys.exit(1)
    ...
    console.print(
        f"  Verbs matched: {len(comparison['verbs_match']['common'])}/{len(comparison['verbs_match']['ground_truth'])}"
    )
    console.print(
        f"  Numbers matched: {len(comparison['numbers_match']['common'])}/{len(comparison['numbers_match']['ground_truth'])}"
    )
    ...
    if ocr_healing.get("has_healing") and gt_healing.get("has_healing"):
        ...
        console.print(
            "  [yellow]⚠ OCR thinks it's healing, but ground truth says it's not[/yellow]"
        )
    ...
    found_important = [
        ...
        if ocr_info["key_phrases"].get(p.replace("_", ""), False) or p in ocr_text.lower()
    ]

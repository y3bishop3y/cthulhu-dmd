...
def move_season1_characters():
    ...
    with Progress(
        ...
        for item in items:
            if item.is_dir():
                ...
                if dest.exists():
                    console.print(f"[yellow]Warning: {dest} already exists. Skipping {item.name}[/yellow]")
                else:
    ...
    try:
        ...
        console.print("[yellow]Warning: characters/ directory not empty or cannot be removed[/yellow]")
...
def verify_structure():
    ...
    for season_name in seasons:
        ...
        character_dirs = [d for d in season_dir.iterdir() if d.is_dir() and d.name not in ["enemies", "missions", "animation"]]
...
def main():
    ...
    if annotated_files:
        ...
        action = Confirm.ask(
            f"Found {len(annotated_files)} *_annotated.png files. Archive or delete?",
            default=True
        )
        if action:
    ...
    if ocr_files:
        ...
        action = Confirm.ask(
            f"Found {len(ocr_files)} *_ocr_preprocessed.png files. Archive or delete?",
            default=True
        )
        if action:

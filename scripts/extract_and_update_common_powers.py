...
from typing import Final, Dict, List, Any, Optional, Tuple
...
def extract_text_from_image(image_path: Path) -> str:
    ...
    try:
        ...
        # Use Tesseract with optimized config for card text
    ...
    for power_name in COMMON_POWERS:
        ...
        for extraction in extractions:
            for level_data in extraction:
                ...
                alpha_ratio = len([c for c in description if c.isalpha()]) / max(len(description), 1)
                is_valid = (
...
def main(data_dir: Path, sample_size: int, dry_run: bool):
    ...
    with Progress(
        ...
        for char_dir in sample_dirs:
            ...
            for power_name in COMMON_POWERS:
                levels = parse_power_levels_from_text(text, power_name)
                if levels:
    ...
    if not dry_run:
        ...
        if common_powers_data:
            ...
            for level in sample["levels"][:2]:
                desc = level["description"][:80] + "..." if len(level["description"]) > 80 else level["description"]
    ...
    if remaining_dirs:
        console.print(f"[cyan]Extracting powers from {len(remaining_dirs)} additional characters...[/cyan]")
        with Progress(
            SpinnerColumn(),
            TextColumn("[progress.description]{task.description}"),
            BarColumn(),
    ...
    console.print(f"[green]✓ Found power assignments for {len(character_powers_map)} characters[/green]")
            ...
            for char_file in character_files:
                ...
                if powers:
                    try:
                        ...
                        console.print(f"[yellow]Warning: Could not update {char_file}: {e}[/yellow]")

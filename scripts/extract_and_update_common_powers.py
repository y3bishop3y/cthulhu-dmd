...
def cleanup_common_powers_json(data_dir: Path, dry_run: bool) -> None:
    ...
    for power in powers_data:
        ...
        for level_data in power["levels"]:
            ...
            if cleaned_desc != original_desc:
                ...
                console.print(f"    After:  {cleaned_desc[:100]}{'...' if len(cleaned_desc) > 100 else ''}")
    ...
    if updated_count > 0:
        if dry_run:
            ...
            console.print(f"[green]✓ Updated {updated_count} descriptions in {common_powers_path}[/green]")
    else:

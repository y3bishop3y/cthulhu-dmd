...
HealingEffect:
    ...
    'heal' quantity=INT ('healing' 'points' | 'points')  // Rest action: 3 healing points
;
...
class HealingEffect(BaseModel):
    wounds: int = 0
    stress: int = 0
    healing_points: int = 0  # For Rest action (3 points distributable)
    
    @computed_field
    @property
    def total_healing(self) -> int:
        """Total healing capacity (wounds + stress + healing points)."""
        return self.wounds + self.stress + self.healing_points
